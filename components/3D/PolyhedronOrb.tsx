...
export function PolyhedronOrb({
  ...
  const getSinuousPosition = (progress: number): [number, number, number] => {
    ...
    if (t <= 0.55) {
      ...
      return [
        ...
        2.0 + Math.sin(easeInOutCubic(0.35) * Math.PI * 0.5) * 0.2 - 1.5 * phase2,
        -2.0,
      ];
    }
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      const [x, y, z] = getSinuousPosition(transitionProgress);
      ...
      groupRef.current.scale.setScalar(targetScale * scale);
    }
    ...
    if (hologramRef.current) {
      ...
      const colorTransitionStart = 0.5;
      const colorProgress = Math.max(0, Math.min(1, (transitionProgress - colorTransitionStart) / (1 - colorTransitionStart)));
      hologramRef.current.uniforms.colorMix.value = colorProgress;
    }
    ...
    if (essencePointsRef.current && essencePointsRef.current.material instanceof THREE.ShaderMaterial) {
      ...
      essencePointsRef.current.material.uniforms.opacity.value = 1;
    }
    ...
    if (innerCoreRef.current) {
      ...
      if (innerCoreRef.current.material instanceof THREE.MeshBasicMaterial) {
        innerCoreRef.current.material.opacity = pulse;
      }
    }
