...
export function PolyhedronOrb({
  ...
  const holographicShader = useMemo(() => ({
    uniforms: { time: { value: 0 }, opacity: { value: 1.0 }, colorMix: { value: 0.0 } },
    vertexShader: holographicVertexShader, fragmentShader: holographicFragmentShader,
  }), []);
  ...
  const essenceMaterial = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      vertexShader: essenceVertexShader, fragmentShader: essenceFragmentShader,
  ...
  const getSinuousPosition = (progress: number): [number, number, number] => {
    ...
    if (t <= 0.35) {
      ...
      return [-1.8 * phase1, 2.0 + Math.sin(phase1 * Math.PI * 0.5) * 0.2, -2.0 * phase1];
    }
    if (t <= 0.55) {
      const phase2 = (smoothT - easeInOutCubic(0.35)) / (easeInOutCubic(0.55) - easeInOutCubic(0.35));
      return [-1.8 - (finalX - (-1.8)) * 0.4 * phase2, 2.0 + Math.sin(easeInOutCubic(0.35) * Math.PI * 0.5) * 0.2 - 1.5 * phase2, -2.0];
    }
    const phase3 = (smoothT - easeInOutCubic(0.55)) / (1 - easeInOutCubic(0.55));
    const currentX = -1.8 - (finalX - (-1.8)) * 0.4;
    const currentY = 2.0 + Math.sin(easeInOutCubic(0.35) * Math.PI * 0.5) * 0.2 - 1.5;
    return [currentX + (finalX - currentX) * phase3, currentY + (finalY - currentY) * phase3, -2.0 + (finalZ - (-2.0)) * phase3];
  };
  ...
  useFrame((state) => {
    ...
    if (groupRef.current) {
      ...
      const easeInOut = calculatedProgress < 0.5 ? 4 * calculatedProgress * calculatedProgress * calculatedProgress : 1 - Math.pow(-2 * calculatedProgress + 2, 3) / 2;
      const targetScale = 1 - easeInOut * 0.3;

      groupRef.current.scale.lerp(new THREE.Vector3(targetScale * scale, targetScale * scale, targetScale * scale), 0.1);
    }

    if (hologramRef.current) {
      hologramRef.current.uniforms.time.value = time;
      hologramRef.current.uniforms.opacity.value = 1;
      hologramRef.current.uniforms.colorMix.value = Math.max(0, Math.min(1, (calculatedProgress - 0.5) / 0.5));
    }
    if (essencePointsRef.current && essencePointsRef.current.material instanceof THREE.ShaderMaterial) {
  ...
  return (
    <group ref={groupRef}>
      <mesh ref={orbRef}>
        ...
        <shaderMaterial ref={hologramRef} {...holographicShader} transparent side={THREE.DoubleSide} depthWrite={false} />
      </mesh>
      <points ref={essencePointsRef} material={essenceMaterial}>
        <bufferGeometry>
          <bufferAttribute attach="attributes-position" args={[essenceParticles.positions, 3]} />
          <bufferAttribute attach="attributes-scale" args={[essenceParticles.scales, 1]} />
          <bufferAttribute attach="attributes-randomness" args={[essenceParticles.randomness, 3]} />
          <bufferAttribute attach="attributes-color" args={[essenceParticles.colors, 3]} />
        </bufferGeometry>
      </points>
      ...
      <pointLight ref={lightRef} position={[0, 0, 0]} color="#8fb3c4" intensity={3} distance={20} />
      <pointLight position={[0, 0, 0]} color="#ffd700" intensity={1.5} distance={8} />
    </group>
  );
}